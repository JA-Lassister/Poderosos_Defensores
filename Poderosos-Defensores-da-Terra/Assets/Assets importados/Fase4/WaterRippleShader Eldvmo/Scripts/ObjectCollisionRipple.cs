...
namespace Eldvmo.Ripples
{
    public class ObjectCollisionRipple : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (Physics.Raycast(origin, direction, out RaycastHit hit, 2f, waterLayerMask))
            {
                Vector2 uv = hit.textureCoord;

                if (_oldInputCentre != null && Vector2.Distance(_oldInputCentre, uv) < 0.05f)
                    return;
        ...
        private void SetObjectHeight(float targetHeight)
        {
            ...
            // Move using Rigidbody (compatible with kinematic bodies)
